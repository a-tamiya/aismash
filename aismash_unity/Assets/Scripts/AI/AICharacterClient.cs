    ...
    public static class AICharacterClient
    {
        ...
        static string BuildBody(string prompt)
        {
            string esc = prompt
                ...
                .Replace("\r", "")
                .Replace("\t", " ");
            return $"{{\"model\":\"{Model}\",\"response_format\":{{\"type\":\"json_object\"}},\"messages\":[{{\"role\":\"user\",\"content\":\"{esc}\"}}]}}";
        }
...
- 【新アクション types】
  ...
  summon: duration=召喚体の寿命(1〜6)、power=パトロール速度(0.5〜5)、spawn_x/y=出現位置、damage_override=接触ダメージ。召喚体が自動でパトロールしながら攻撃する。召喚するものの見た目はその技スロットのeffect画像として生成される前提で、skill_name/descriptionに召喚物の名前や形状を明確に含める。recovery 0.10〜0.35（召喚直後すぐ動ける）
