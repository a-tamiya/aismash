...
- features: そのキャラの『見た目・外見』『性格・特徴』『ステータス傾向（素早い/重い/打たれ強い/紙耐久など）』『戦い方・間合い（近接/遠距離/設置/召喚/カウンター等）』『技の内容（どんな攻撃をするか）』を具体的に描いた日本語の文章（90〜160字）。後でこの文章を素材に技を自動生成するため、見た目・体格・武器・戦法・各技がどんな効果を持つかを手がかりとして盛り込む。
- 技名（固有の必殺技名）は出力しない。技は名前ではなく『どんな技か』を内容で説明する。
- 毎回まったく異なるタイプのキャラにする。定番に寄せず、与えられたテーマ種を起点に唯一無二の個性・意外な組み合わせを作る。
