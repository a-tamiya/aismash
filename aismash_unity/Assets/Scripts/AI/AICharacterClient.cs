...
- 【派生技 follow_up_actions】attack_a専用でスキルJSON最上位に追加:
  attack_aは近接パンチ1発にする。actionsはtime:0のmelee_hitbox/body_hitboxを1つだけにし、projectileやdashを入れない
  follow_up_actionsはtime:0の近接蹴り1発(melee_hitbox)にする。follow_up_windowは0.6
  派生はヒット/空振りに関係なく、受付時間内に同じボタンを押すと最大3回まで出る。派生中はUnity側で最終段以外を弱く引き寄せる
  attack_b/attack_c/smash_sideはfollow_up_actionsを付けない普通の攻撃にする
- 【チャージ技】スキルJSON最上位に追加:
  chargeable: true  ← ボタン長押しで威力上昇(1.0〜1.8倍)
  max_charge_time: 0.3〜1.5  ← フルチャージ時間(秒)
  ★ chargeable:trueはattack_b またはattack_cに設定すること（attack_aはスマッシュと競合するため不可）
...
{{
  ...
  ""skills"": [
    {{
      ...
      ""actions"": [
        {{""type"":""melee_hitbox"",""time"":0.0,""range"":[0.8〜1.6],""spawn_x"":[0.55〜1.1],""spawn_y"":[0.55〜1.1],""size_y"":[0.8〜1.2],""hit_count"":1}}
      ],
      ""follow_up_actions"": [
        {{""type"":""melee_hitbox"",""time"":0.0,""range"":[0.9〜1.8],""spawn_x"":[0.65〜1.25],""spawn_y"":[0.05〜0.45],""size_y"":[0.75〜1.15],""hit_count"":1}}
      ],
      ""follow_up_window"": 0.6
    }},
    {{
      ""slot"": ""attack_b"",
