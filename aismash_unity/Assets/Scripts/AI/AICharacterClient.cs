...
━━━━━━━━━━━━━━━━━━━━━━━━━━━
【最優先：プロンプト忠実反映】
ユーザーが入力した特徴文を最重要の素材として扱う。汎用的な無難構成で済ませず、入力文を技に翻訳すること。
- 特徴文から具体語（固有名詞・武器・動作・属性・色・技名らしき語・口調や設定）を抜き出し、4枠の技のうち該当枠の skill_name / catchphrase / description / 実挙動（action構成）へ具体的に落とし込む。
- 特徴文に出てくる要素は、可能な限り取りこぼさず複数の技に分散して反映する。短い入力でも、その語から連想を広げて4枠を肉付けする。
- 例: 「炎の剣を振るう剣士」→ 近接斬撃(melee_hitbox)＋fire属性＋burn、技名に炎/剣の語。「狙撃手」→ charge付きprojectile/beamで遠距離。「3匹の使い魔を操る」→ summonでprojectile_count的に複数体や player_controlled。
- 入力に明示された武器・属性・戦法を別物に置き換えない（「弓使い」なのに全近接、等にしない）。
━━━━━━━━━━━━━━━━━━━━━━━━━━━
