...
- 補助・攪乱タイプ → buff_selfやteleportを使ってよい。buff_selfのstatusは speed / jump / invincible / damage から選ぶ
...
- 置き技は trap_hitbox を使い、duration 0.8〜3.0、size_x/size_y、spawn_x/spawn_y を必ず指定する。本人のrecoveryはtrapのdurationより短くしてよい。
- 多段攻撃はhit_countを2〜6にし、damageは1ヒットあたり低くする。合計火力が単発技を大きく超えないようにする。
...
{{
  ...
  ""skills"": [
    ...
    {{
      ...
      ""actions"": [
        {{""type"":""trap_hitbox"",""time"":[推論],""duration"":[0.8〜3.0],""spawn_x"":[0.8〜2.6],""spawn_y"":[0.05〜1.2],""size_x"":[0.8〜3.2],""size_y"":[0.5〜2.0],""hit_count"":[1〜3],""status"":""[属性に合う状態異常]"",""chance"":[推論]}}
      ]
    }},
    {{
      ""slot"": ""smash_side"",
